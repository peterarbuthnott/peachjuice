...
// the top, tapered "returns" down the left/right sides (for a bit of
// perspective, like looking into a shallow stage set), and a window that
// simply doesn't accept paint. CEILING_H/EDGE_W define the actual
// paintable boundary; TOP_EDGE_W is only used to taper the roof/side
// artwork so it lines up with that boundary rather than floating free of
// it.
// ----------------------------------------------------------------------------
const CEILING_H = 44;
const EDGE_W = 46; // width of the side returns at the BOTTOM of the wall (also the paintable x-boundary used for coverage sampling)
const TOP_EDGE_W = 24; // narrower width at the TOP of the wall - this taper is the "perspective"
...
// Coverage is sampled from the INNER rectangle (a safe under-estimate of
// the true, tapered paintable area - see leftEdgeAt() below - fully
// inside it at every y, so it never counts anything as "paintable" that
// actually isn't). The window is carved out of that rectangle too, so a
// player is never required to paint the glass to reach 100%.
...
// ----------------------------------------------------------------------------
// The paintable x-boundary tapers with y - narrower (TOP_EDGE_W) near the
// roof, widening to EDGE_W by the time it reaches the skirting board -
// this is what gives the side "returns" their perspective.
function leftEdgeAt(y) {
  const t = Math.max(0, Math.min(1, (y - CEILING_H) / (WALL_H - CEILING_H)));
  return TOP_EDGE_W + (EDGE_W - TOP_EDGE_W) * t;
}
...
function isPaintable(x, y) {
  ...
  const left = leftEdgeAt(y);
  if (x < left || x > CANVAS_W - left) return false;
...
function clampToWall(pt) {
  const y = Math.max(CEILING_H, Math.min(WALL_H, pt.y));
  const left = leftEdgeAt(y);
  return { x: Math.max(left, Math.min(CANVAS_W - left, pt.x)), y };
}
...
function drawCeiling() {
  ...
  ctx.lineTo(CANVAS_W - TOP_EDGE_W, CEILING_H);
  ctx.lineTo(TOP_EDGE_W, CEILING_H);
  ctx.closePath();
  const g = ctx.createLinearGradient(0, 0, 0, CEILING_H);
  g.addColorStop(0, '#cfc6b2');
  g.addColorStop(1, '#a89e88');
  ctx.fillStyle = g;
  ctx.fill();
...
// The one bit of actual game design here: "watching" means the mouse is
// literally hovering over the painted wall, not just that the browser tab
// happens to be open. document.visibilityState is still checked too, so
// switching tabs/apps also correctly pauses the clock (the pointer can't
// meaningfully be "over" a wall that isn't even on screen).
function isCurrentlyWatching() {
  return document.visibilityState === 'visible' && state.pointerOverCanvas;
}
