...
// Backfills momentsOfCalm for entries saved before this field existed
// (back when the leaderboard ranked fastest totalElapsedMs instead) --
// best-effort from whatever watchedMs they already recorded, so old
// scores don't sort as NaN/last once the new field is introduced.
function migrateEntry(e) {
  if (!isPlainNumber(e.momentsOfCalm)) {
    e.momentsOfCalm = isPlainNumber(e.watchedMs) ? Math.floor(e.watchedMs / 1000) : 0;
  }
  return e;
}
...
function handleApi(req, res, pathname) {
  if (pathname === '/api/score' && req.method === 'POST') {
    readBody(req, function (err, body) {
      ...
      if (!body || typeof body.playerId !== 'string' || !isPlainNumber(body.momentsOfCalm)) {
        return sendJSON(res, 400, { error: 'playerId (string) and momentsOfCalm (number) are required' });
      }
      ...
      var entry = {
        ...
        momentsOfCalm: Math.round(body.momentsOfCalm),
        paintBonusAwarded: !!body.paintBonusAwarded,
        totalElapsedMs: isPlainNumber(body.totalElapsedMs) ? Math.round(body.totalElapsedMs) : null,
      ...
      var highscores = loadJSON(HIGHSCORES_FILE, []).map(migrateEntry);
      highscores.push(entry);
      // Most Moments of Calm first - highest score wins. This is
      // deliberately NOT the fastest run: since the drying clock in
      // game.js now runs unconditionally, the score instead rewards
      // however much of that unavoidable drying time the player actually
      // sat and watched (plus the flat in-time painting bonus).
      highscores.sort(function (a, b) { return b.momentsOfCalm - a.momentsOfCalm; });
      ...
      sendJSON(res, 200, { ok: true, momentsOfCalm: entry.momentsOfCalm });
    });
    return;
  }
  ...
  if (pathname === '/api/highscores' && req.method === 'GET') {
    ...
    var all = loadJSON(HIGHSCORES_FILE, []).map(migrateEntry);
    all.sort(function (a, b) { return b.momentsOfCalm - a.momentsOfCalm; });
    ...
    for (var i = 0; i < page.length; i++) {
      ...
      ranked.push({
        ...
        momentsOfCalm: e.momentsOfCalm,
        paintBonusAwarded: e.paintBonusAwarded,
        totalElapsedMs: e.totalElapsedMs,
        formattedTime: isPlainNumber(e.totalElapsedMs) ? formatHms(e.totalElapsedMs) : null,
