...
const PAINT_TIME_LIMIT_MS = 60 * 1000; // round timer for the painting phase
...
// and startWatchingBtn's paintBonusAwarded check.
...
// Width is constant (EDGE_W) top to bottom, lining up exactly with the
// ceiling's bottom edge and the floor's top edge.
function drawEdges() {
  ctx.save();
  ...
  ctx.fillStyle = g;
  ctx.fillRect(0, CEILING_H, EDGE_W, WALL_H - CEILING_H);

  g = ctx.createLinearGradient(CANVAS_W - EDGE_W, 0, CANVAS_W, 0);
  g.addColorStop(0, '#c7bfae');
  g.addColorStop(1, '#8f8879');
  ctx.fillStyle = g;
  ctx.fillRect(CANVAS_W - EDGE_W, CEILING_H, EDGE_W, WALL_H - CEILING_H);
  ctx.restore();
}
...
function drawFloor() {
  ...
  // Skirting board highlight/shadow lines, following the same trapezoid.
  ...
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(1, CANVAS_H - 2);
  ctx.lineTo(CANVAS_W - 1, CANVAS_H - 2);
  ctx.stroke();
  ctx.restore();
}
...
// costs the completion bonus (see PAINT_TIME_BONUS / startWatchingBtn).
...
function updateWatchStats() {
  watchStats.textContent = `Moments of calm: ${Math.floor(state.watchedMs / 1000)} · Looked away ${formatMinSec(state.awayMs)} (${state.lookAwayCount}x)`;
}
