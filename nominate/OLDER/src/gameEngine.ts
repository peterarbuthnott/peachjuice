import { GameState, PlayerState, PlayerRoundScore, RoundInfo } from './types';

const TRUMP_SUITS = ["clubs ♣", "diamonds ♦", "hearts ♥", "spades ♠"];
const RED_SUITS = [false, true, true, false];

export function getRoundInfo(roundNumber: number): RoundInfo {
  let tricks = 0;
  let trumpSuit = "no";
  let isRedSuit = false;
  let isMiss = false;
...
export function shiftDealer(players: PlayerState[]): PlayerState[] {
...
export function computeScoredPlayers(
  players: PlayerState[],
  roundNumber: number,
  isMiss: boolean
): PlayerState[] {
