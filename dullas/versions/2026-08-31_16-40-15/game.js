...
const STACK_PLATE_SPACING = 9; // tightened from 12 so the full 40-plate stack still fits above the bottom upgrade overlay
...
// Mirrors the #upgrade-panel height in style.css - now that upgrades render
// as a bottom overlay bar (like the HUD does at the top), the stacks need
// to stay clear of it, the same way they already stay clear of the HUD.
const UPGRADE_OVERLAY_HEIGHT = 92;

const STACK_CONTAINER = { w: STACK_SLOT_COUNT * STACK_COLUMN_WIDTH };
STACK_CONTAINER.h = STACK_CAPACITY * STACK_PLATE_SPACING + 40; // fits the play field, with room for the heading
STACK_CONTAINER.x = CANVAS_W - 20 - STACK_CONTAINER.w;
STACK_CONTAINER.y = CANVAS_H - 20 - UPGRADE_OVERLAY_HEIGHT - STACK_CONTAINER.h;
