...
function drawUpgradesPanel() {
  ...
  UPGRADE_DEFS.forEach((def, index) => {
    ...
    ctx.fillStyle = '#ffffff';
    ctx.fillText(def.name, item.x + 8, item.y + 14);
    ctx.font = '10px Segoe UI, Arial, sans-serif';
    ctx.fillStyle = '#9fd8b8';
    ctx.fillText(`Lv. ${level} / ${maxLevelLabel}`, item.x + 8, item.y + 27);
    ctx.fillStyle = '#cfe0d8';
    ctx.fillText(def.currentValueLabel(), item.x + 8, item.y + 40);
    ctx.globalAlpha = 1;
    ctx.restore();
    ...
    if (!unlocked) {
      ...
      label = `Buy - ${cost}`; kind = affordable ? 'buy' : 'unaffordable';
    }
