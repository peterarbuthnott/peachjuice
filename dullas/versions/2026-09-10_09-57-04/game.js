...
// block audio until a real user gesture, so ensureAudioContext() is only
// ever called from inside click handlers (Start Washing / Continue).
// ============================================================================
let audioCtx = null;
let masterGain = null;
...
// A second or so of white noise, generated once and reused (looped) for
// the scrubbing "swish" - regenerating it every time would be wasteful.
...
// "bling" - a golden plate appears: three shimmering high tones.
function playBling() {
  playTone({ freq: 2600, type: 'sine', duration: 0.22, gain: 0.10 });
  playTone({ freq: 3300, type: 'sine', duration: 0.20, gain: 0.08, start: 0.03 });
  playTone({ freq: 4100, type: 'sine', duration: 0.18, gain: 0.06, start: 0.06 });
}
...
// The scrubbing "swish" is continuous rather than a one-shot: a looping
// noise buffer through a bandpass filter, with its gain smoothly glided
// toward a quiet target whenever handleMove() reports an actual scrub
// against the plate (state.lastScrubSoundAt), and toward silence otherwise.
// updateSwishSound() is called once per frame from tick().
let swishSource = null;
let swishFilter = null;
let swishGain = null;

function ensureSwishNodes() {
  if (!audioCtx || swishSource) return;
  swishSource = audioCtx.createBufferSource();
  swishSource.buffer = getNoiseBuffer();
  swishSource.loop = true;
  swishFilter = audioCtx.createBiquadFilter();
  swishFilter.type = 'bandpass';
  swishFilter.frequency.value = 1200;
  swishFilter.Q.value = 0.6;
  swishGain = audioCtx.createGain();
  swishGain.gain.value = 0;
  swishSource.connect(swishFilter).connect(swishGain).connect(masterGain);
  swishSource.start();
}

function updateSwishSound() {
  if (!audioCtx) return;
  ensureSwishNodes();
  const scrubbingNow = performance.now() - state.lastScrubSoundAt < 120;
  // "quiet swish" - kept well below the one-shot cues' volume. Continuous
  // sounds read as much louder than transient tones at the same gain, so
  // this is set noticeably lower than the momentary cue gains above.
  swishGain.gain.setTargetAtTime(scrubbingNow ? 0.018 : 0, audioCtx.currentTime, 0.05);
}
...
const state = {
  ...
  lastScrubSoundAt: 0, // performance.now() of the last successful scrubAt() hit, drives the swish sound - see updateSwishSound()
...
function handleMove(x, y) {
  ...
  if (!state.roundComplete && state.activePlate) {
    ...
    if (scrubbed) {
      ...
      // corners outside the round plate. The swish sound should only play
      // while actually over the dish, so gate it on real distance from the
      // plate's center against its circular radius.
      const overPlate = Math.hypot(x - plate.cx, y - plate.cy) <= plate.radius;
      if (overPlate) {
        state.lastScrubSoundAt = performance.now(); // drives the "swish" sound - see updateSwishSound()
      }
