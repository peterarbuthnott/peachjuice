...
function updatePlateAnimation() {
  ...
  if (t >= 1) {
    state.animatingPlate = null;
    state.platesCleaned += anim.plate.isGolden ? GOLD_REWARD : NORMAL_REWARD;
...
function drawRack() {
  ...
  for (let i = 0; i < shown; i++) {
    const idx = total - shown + i; // oldest-of-the-visible-set first, newest on top
    const isGolden = state.rackPlates[idx];
  ...
  if (total > shown) {
    ...
    ctx.fillText(`+${total - shown}`, RACK_CENTER_X, RACK_BOX.y + 32);
  }
  ctx.textAlign = 'left';
  ctx.restore();
}
