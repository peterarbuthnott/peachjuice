...
// holds up to 4 stacks side by side, each capped at 40 plates. Stacks are
// cleared at the start of every round. If a round somehow needs a 5th
// stack, the view slides: the oldest stack drops off and every other one
// shifts one slot over, so the newest stack always lands in the last slot.
// ----------------------------------------------------------------------------
const STACK_CAPACITY = 40;
const STACK_SLOT_COUNT = 4;
const STACK_PLATE_SPACING = 12;
const STACK_COLUMN_WIDTH = 90;
...
STACK_CONTAINER.h = STACK_CAPACITY * STACK_PLATE_SPACING + 40; // fits the window height exactly
...
// Which of the STACK_SLOT_COUNT visible columns a given stack currently
// occupies. Once there are more stacks than slots, everything shifts left
// (in index terms) so the newest stack is always in the last slot and the
// oldest ones scroll off (return value < 0, meaning "don't draw it").
function stackSlotForIndex(stackIndex) {
  const shift = Math.max(0, state.stacks.length - STACK_SLOT_COUNT);
  return stackIndex - shift;
}
...
function stackPlatePosition(stackIndex, plateIndexInStack) {
  const slot = stackSlotForIndex(stackIndex);
  return {
    x: stackColumnX(slot) + STACK_COLUMN_WIDTH / 2,
...
// starts a new one and pays out the fill bonus.
...
const state = {
  ...
  stacks: [[]], // one array per drying stack, each holding one bool (isGolden) per plate; cleared each round
  isScrubbing: false,
  mouseX: CANVAS_W / 2,
  mouseY: CANVAS_H / 2,
  spongeRadius: 34,
...
const UPGRADE_DEFS = [
  {
    ...
    currentValueLabel: () => `${Math.round(state.spongeRadius)}px radius`,
  },
  {
    id: 'efficientSponge',
    ...
    isMaxed: () => (state.upgrades.efficientSponge || 0) >= 11,
    currentValueLabel: () => `${Math.round(state.scrubEfficiency * 100)}% strength`,
  },
  {
    id: 'doublePlates',
    name: 'Extra Dishes',
...
function startRound(roundNum) {
  ...
  state.stacks = [[]]; // drying stacks are cleared out at the start of every round
...
// Drying stacks: one shared container backdrop (no title, per design), with
// up to STACK_SLOT_COUNT stacks of plates shown side by side inside it.
// Stacks are drawn oldest-first, but only the ones currently in a visible
// slot (see stackSlotForIndex) actually render - older ones scroll off.
function drawStacks() {
  ctx.save();
  ...
  state.stacks.forEach((stack, stackIndex) => {
    const slot = stackSlotForIndex(stackIndex);
    if (slot < 0 || slot >= STACK_SLOT_COUNT) return;
